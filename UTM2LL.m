function [LatDeg, LongDeg] = UTM2LL(LongOrigin_Old, North_Old , East_Old)
    ...
    LatDeg  = (phi1Rad - (N1.*tan(phi1Rad).*((1 - WGS84_ECC_SQ*sin(phi1Rad).*sin(phi1Rad)).^1.5)/(WGS84_A * (1-WGS84_ECC_SQ))) ...
                        .* (D.*D/2 - (5 + 3*T1 + 10*C1 - 4*C1.*C1 - 9*EE).*D.*D.*D.*D/24 ...
                        + (61 + 90*T1 + 298*C1 + 45*T1.*T1 - 252*EE - 3*C1.*C1).*D.*D.*D.*D.*D.*D/720)) * 180/pi;
    LongDeg = LongOrigin_Old + ((D - (1 + 2*T1 + C1).*D.*D.*D/6 ...
